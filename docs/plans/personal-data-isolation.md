...
|---|---|---|---|
| **D1** | SOUL.md / STYLE.md / AGENTS.md — what ships in the repo? | **Placeholder + runtime override** | Repo ships sparse template versions (~15 lines each: section headers + one-line placeholders). `src/core/CLAUDE.md` chains the @-includes: repo placeholder loads first as baseline, then `@~/.construct/identity/X.md` appends user customization if present. Fresh install gets a coherent generic baseline; the author keeps his real SOUL/STYLE/AGENTS at `~/.construct/identity/` where they always loaded from anyway. AGENTS.md keeps its workflow framework (depth classification, persona selection) verbatim — only personal-style wording bits (`<SUBAGENT-STOP>` tone, "1% rule" framing) get genericized. **Open sub-question:** does Claude Code's @-include expand `~/`? If not, `install.ts` rewrites `~/` → `$HOME` at install time. |
...
| 8 | Per D1: rewrite SOUL/STYLE/AGENTS to ~15-line placeholder versions (keep AGENTS.md workflow framework, strip personal wording). Add three more @-includes to `src/core/CLAUDE.md` pointing at `~/.construct/identity/{SOUL,STYLE,AGENTS}.md`. Verify @-include `~/` expansion; if not supported, add path-rewrite to `install.ts`. Manually `cp` author's existing SOUL/STYLE/AGENTS to `~/.construct/identity/` so the override takes effect. | `src/core/identity/SOUL.md`, `STYLE.md`, `AGENTS.md`, `src/core/CLAUDE.md`, possibly `install.ts` |
