...
|---|---|---|---|
| **D1** | SOUL.md / STYLE.md / AGENTS.md — what ships in the repo? | **Base + extend, `.override.md` naming** | Repo ships full base versions (real working content, minus personal bits). User-side override files at `~/.construct/identity/{AGENTS,SOUL,STYLE}.override.md` are SHORT — just the diff from base. USER.md is asymmetric: no repo base, so user-side stays `~/.construct/identity/USER.md` (no `.override` suffix — nothing to override). `src/core/CLAUDE.md` chains @-includes: bases first, overrides append, USER last. Naming aligns with the AGENTS.md ecosystem standard (`AGENTS.override.md` is the canonical co-loaded override per the Linux Foundation-stewarded spec); same convention applied to SOUL/STYLE for consistency. AGENTS.md walkthrough identified four lines to extract: "Don't ask 'shall I proceed?'" (line 80), Haiku/Sonnet/Opus model table (lines 93-99), Daily rhythm (lines 109-112), "Squash when merging" (line 131). SOUL.md and STYLE.md need separate walkthroughs. **Open sub-question:** does Claude Code's @-include expand `~/`? If not, `install.ts` rewrites `~/` → `$HOME` at install time. **Override discipline:** the user file is the diff, not a copy of the base — copying full base into override locks the user out of future base updates (BMAD's documented anti-pattern). |
...
| 8 | Per D1: extract personal lines from SOUL/STYLE/AGENTS (keep the rest as the base). Add three more @-includes to `src/core/CLAUDE.md` pointing at `~/.construct/identity/{AGENTS,SOUL,STYLE}.override.md` (note `.override.md` suffix — matches AGENTS.md ecosystem spec). Verify @-include `~/` expansion; if not supported, add path-rewrite to `install.ts`. Place the extracted personal content into `~/.construct/identity/{AGENTS,SOUL,STYLE}.override.md` so the user's override loads. Must land atomically (same commit removes from base AND adds chain include) or the override file must exist before the base is trimmed. USER.md stays bare-named (no `.override` suffix) since it has no repo base — `@~/.construct/identity/USER.md`. | `src/core/identity/SOUL.md`, `STYLE.md`, `AGENTS.md`, `src/core/CLAUDE.md`, possibly `install.ts` |
